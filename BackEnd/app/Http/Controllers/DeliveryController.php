...
{
    public function index($type){//type => deliverings , type => delivered
        $transletCountry = [
    ...
    public function viewregister($type){// type => delivered , type => completed
        $transletCountry = [
            'uae' => 'الإمارات العربية المتحدة',
            'turkey' => 'تركيا',
            'china' => 'الصين',
        ...
        $orders = Order::with('customer','commission')
            ->select('id','order_number','weight','shipping_cost','commission_id','expected_delivery_time','created_at')
            ->where('status',$type)
            ->get();
        $orders = $orders->map(function($order)use($transletCountry,$days){
            ...
            return [
                'id' => $order->id,
                'order_number' => $order->order_number,
                'cost' => (float)($order->weight * $order->commission->price_per_kg) + $order->shipping_cost + $order->commission->commission,
                'country' => strtr($order->commission->country,$transletCountry),
                'order_date' => $createdAt ? $createdAt->format('Y-m-d') : null, // تاريخ الطلب
                'delivery_date' => $formattedDeliveryDate, // تاريخ التسليم المتوقع
                'delivery_day' => $days[$dayOfWeek] // يوم التسليم المتوقع
